...
def make_mathml_accessible(html_content: str) -> str:
    """
    Finds <math> elements in HTML and enriches them with alttext and aria-label
    extracted from their LaTeX annotation. Also wraps block math in scrollable containers.
    """
    ...
    def replace_math(match):
        ...
        latex_match = re.search(r'<annotation[^>]*encoding="application/x-tex"[^>]*>(.*?)</annotation>', inner_content, re.DOTALL)
        if latex_match:
            latex_code = html.unescape(latex_match.group(1).strip())
            escaped_latex = html.escape(latex_code).replace('"', '&quot;')
            
            if 'alttext' not in tag_attrs:
                tag_attrs_clean = tag_attrs.rstrip(' >')
                new_open_tag = f'<math {tag_attrs_clean} alttext="Formula: {escaped_latex}" aria-label="Formula: {escaped_latex}">'
            else:
                new_open_tag = full_open_tag
        else:
            new_open_tag = full_open_tag
        ...
        if display_type == "block":
            return (
                f'<div class="math-container" tabindex="0" role="group" '
                f'aria-label="Mathematical formula. Use arrow keys to scroll.">{math_block}</div>'
            )
        return math_block
...
def process_markdown_equations(content: str) -> str:
    """Find and replace all SymPy and MathML formulas in markdown with standard LaTeX math."""
...
def main():
    ...
    if target_format not in ("word", "html", "pdf", "epub", "md", "markdown", "daisy"):
        print(f"Error: Unknown output format '{target_format}'. Must be 'word', 'html', 'pdf', 'epub', 'md', or 'daisy'.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        converted_content = process_markdown_equations(content)
